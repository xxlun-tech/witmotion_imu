...
{
public:
  Hwt905Driver() = default;

  virtual ~Hwt905Driver() = default;

  Hwt905_DataType_t process_next_byte(uint8_t byte);

  static constexpr const size_t PAYLOAD_SIZE = 11;

  static void linearize_ring_buffer(uint8_t linear_buf[PAYLOAD_SIZE], const uint8_t ring_buf[PAYLOAD_SIZE], size_t ring_buf_idx);
